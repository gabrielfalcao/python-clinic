...
def determine_log_level():
    """attempts to retrieve the log level from the environment variable ``PYTHON_CLINIC_LOGLEVEL``
    """
    from_env_vars = os.getenv('PYTHON_CLINIC_LOGLEVEL')
...
def get_log_filename_from_conf(conf, prefix='python-clinic.%Y-%m-%d.%H-%M'):
...
def get_logger(name, *args, **kw):
    ...
    if name == 'python_clinic' or not name:
        logger.handlers = [handler]

    elif name.startswith('python_clinic.'):
        logger.handlers = []
...
logger = get_logger('python_clinic')
